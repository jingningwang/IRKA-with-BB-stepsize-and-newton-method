...
class AlphaInterpReductor(BasicObject):
    ...
    ----------
    fom
        The full-order |LTIModel| to reduce. 准确的函数
    rom
        Reduced-order |LTIModel|. 要估计的函数          
    sigma
        Interpolation points.  函数作用的点, 即s
    b
    ...
    def _compute_gradients(self):
        Pt, Qt = solve_sylv_schur(self.fom.A,self.rom.A,self.fom.E,self.rom.E,self.fom.B,self.rom.B,self.fom.C,self.rom.C,)
        Pr = self.rom.gramian('c_dense')
        Qr = self.rom.gramian('o_dense')
        Er = to_matrix(self.rom.E, format='dense')  # 转换为矩阵
...
class IRKAWithLineSearchReductor(IRKACauchyIndexReductor):     #主程序
    ...
    def reduce(self,rom0_params,alpha_min=1e-20,tol=1e-16,maxit=1000,num_prev=1,
        ...
        error_old = rom.h2_norm() ** 2 - 2 * np.trace(self.fom.C.apply(Pt).to_numpy().T @ rom.C.matrix.T)

        for it in range(maxit):
            msg = ''
            Er = to_matrix(rom.E, format='dense')  # 转换为矩阵
            Ar = to_matrix(rom.A, format='dense')
            Br = to_matrix(rom.B, format='dense')
            Cr = to_matrix(rom.C, format='dense')
            H2 = LTIModel.from_matrices(Ar, Br, Cr, E=Er)
            alpha_interp = AlphaInterpReductor(self.fom, rom, sigma, b, c, mu=self.mu)
            alpha_interp._compute_gradients()
            # print('aaa')
            # print(alpha_interp._dA,alpha_interp._dB,alpha_interp._dC,alpha_interp._dE)
            global alpha
            alpha=1

            grad2 = LTIModel.from_matrices(alpha_interp._dA, alpha_interp._dB, alpha_interp._dC,E=alpha_interp._dE)
            if it==0:
                alpla=1
            else:
                alpha=(H2-H1).h2_norm()**2 / (((H2-H1)+(grad2-grad1)).h2_norm()**2-(H2-H1).h2_norm()**2-(grad2-grad1).h2_norm()**2)
                print(alpha)
            print(grad2.h2_norm())
            ...
            while alpha >= alpha_min:
                ...
                if rom_new.poles().real.max() < 0:
                    Pt_new = solve_sylv_schur(self.fom.A,rom_new.A,self.fom.E,rom_new.E,self.fom.B,rom_new.B,)
                    error_new = rom_new.h2_norm() ** 2 - 2 * np.trace(
                        self.fom.C.apply(Pt_new).to_numpy().T @ rom_new.C.matrix.T)
                    if error_new < error_old:
                        rom_old=rom
                        Er_old = to_matrix(rom_old.E, format='dense')  # 转换为矩阵
                        Ar_old = to_matrix(rom_old.A, format='dense')
                        Br_old = to_matrix(rom_old.B, format='dense')
                        Cr_old = to_matrix(rom_old.C, format='dense')
                        H1 = LTIModel.from_matrices(Ar_old, Br_old, Cr_old, E=Er_old)
                        gr=AlphaInterpReductor(self.fom,rom_new,sigma,b,c,mu=self.mu)
                        gr._compute_gradients()
                        grad1 = LTIModel.from_matrices(gr._dA, gr._dB, gr._dC, E=gr._dE)
                        rom = rom_new
                        break
                    else:
